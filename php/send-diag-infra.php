...
// ── Corps texte ──────────────────────────────────────────────────────────────
$text  = "Demande d'analyse approfondie — Diagnostic Maturité Infrastructure SI\n\n";
$text .= "Nom    : $name\n";
$text .= "Email  : $email\n";
...
if ($diag) {
    ...
    if (!empty($client['size']))    $text .= "Taille SI     : {$client['size']}\n";
    if (!empty($client['cloud']))   $text .= "Maturité cloud: {$client['cloud']}\n";

    $text .= "\n========================================\n";
    $text .= "RÉSULTATS DU DIAGNOSTIC INFRASTRUCTURE SI\n";
    $text .= "========================================\n";
    ...
    if (!empty($diag['roadmap'])) {
        $text .= "\n========================================\n";
        $text .= "FEUILLE DE ROUTE — 12 MOIS\n";
        $text .= "========================================\n";
        foreach ($diag['roadmap'] as $phase) {
            $text .= "\n{$phase['label']}\n";
...
if ($pdfB64) {
    ...
    $mime .= "Content-Disposition: attachment; filename=\"diag-infra-{$clientSlug}.pdf\"\r\n";
...
if ($csvB64) {
    ...
    $mime .= "Content-Disposition: attachment; filename=\"diag-infra-{$clientSlug}.csv\"\r\n";
    $mime .= "Content-Transfer-Encoding: base64\r\n\r\n";
...
$sent = mail($to, "Diag Infrastructure SI — {$name}", $mime, $headers);
