...
// Configuration email
define('ADMIN_EMAIL', 'recommandation@galynup.fr');
define('WEBSITE_URL', 'https://www.galynup.fr');

// Fonction pour envoyer un email de remerciement
function sendThankYouEmail($email, $name) {
    $subject = 'Votre recommandation a ete publiee sur GALYN\'UP';
    
    $message = "<html><body style='font-family: Arial, sans-serif; color: #333;'>
        <div style='max-width: 600px; margin: 0 auto;'>
            <h2 style='color: #9333ea;'>Merci pour votre recommandation ! </h2>
            <p>Bonjour $name,</p>
            <p>Nous sommes heureux de vous annoncer que votre recommandation a ete approuvee et est maintenant publiee sur notre site.</p>
            <p><strong>Votre recommandation est visible ici :</strong><br>
            <a href='" . WEBSITE_URL . "/#recommandations' style='color: #9333ea; text-decoration: none;'>" . WEBSITE_URL . "/#recommandations</a></p>
            <p>Merci de nous faire confiance et de nous recommander aupres de vos reseaux !</p>
            <p style='margin-top: 2rem; padding-top: 1rem; border-top: 1px solid #eee; font-size: 0.9em; color: #999;'>
                Cordialement,<br>
                <strong>GALYN\'UP - CIO Advisory</strong>
            </p>
        </div>
    </body></html>";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($id > 0) {
        if ($action === 'approve') {
            ...
            // Recuperer les donnees de la recommandation pour envoyer l'email
            $rec_stmt = $conn->prepare("SELECT email, name FROM recommendations WHERE id = ?");
            $rec_stmt->bind_param('i', $id);
            $rec_stmt->execute();
            $rec_result = $rec_stmt->get_result();
            if ($rec_row = $rec_result->fetch_assoc()) {
                // Envoyer l'email de remerciement
                sendThankYouEmail($rec_row['email'], $rec_row['name']);
            }
            
            $message = '✅ Recommandation approuvée et email envoye';
