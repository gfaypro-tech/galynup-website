...
// Stats
$totalApps      = $db->query("SELECT COUNT(*) FROM obr_applications")->fetchColumn();
$completedApps  = $db->query("SELECT COUNT(*) FROM obr_applications WHERE status = 'completed'")->fetchColumn();
$knowledgeCount = $db->query("SELECT COUNT(*) FROM obr_knowledge WHERE is_active = 1")->fetchColumn();
$entretienCount = $db->query("SELECT COUNT(*) FROM obr_applications WHERE hiring_status IN ('entretien','offre')")->fetchColumn();
...
$stmt = $db->prepare("SELECT * FROM obr_applications $where ORDER BY updated_at DESC $limit");
$stmt->execute($params);
