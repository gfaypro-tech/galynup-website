...
if (!$parsed || empty($parsed['questions'])) {
    ...
    $db->prepare("UPDATE obr_applications SET step_current = 3, status = 'matching' WHERE id = ?")->execute([$id]);
    header('Location: ../new-application.php?id=' . $id);
    exit;
}

// Supprimer les anciennes questions
$db->prepare("DELETE FROM obr_dialogue WHERE application_id = ?")->execute([$id]);

// Réinsérer
$qStmt = $db->prepare("INSERT INTO obr_dialogue (application_id, question_order, question) VALUES (?, ?, ?)");
