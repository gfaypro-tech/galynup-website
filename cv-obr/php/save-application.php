...
if ($id > 0) {
    ...
    $stmt = $db->prepare("UPDATE obr_applications SET company = ?, position = ?, job_posting = ?, source_url = ?, step_current = 2, status = 'analysis', updated_at = NOW() WHERE id = ?");
    $stmt->execute([$company, $position, $job_posting, $source_url ?: null, $id]);
    jsonResponse(['success' => true, 'id' => $id]);
} else {
    // Nouvelle candidature
    $stmt = $db->prepare("INSERT INTO obr_applications (company, position, job_posting, source_url, step_current, status) VALUES (?, ?, ?, ?, 2, 'analysis')");
