...
$db   = getDB();
$stmt = $db->prepare("SELECT enrichment_data FROM obr_applications WHERE id = ?");
$stmt->execute([$app_id]);
$app  = $stmt->fetch();
...
foreach ($knowledge_ids as $kid) {
    if ($kid <= 0) continue;
    $stmt = $db->prepare("SELECT id, content, keywords FROM obr_knowledge WHERE id = ? AND is_active = 1");
    $stmt->execute([$kid]);
    $entry = $stmt->fetch();
    if (!$entry) continue;
    ...
    $db->prepare("UPDATE obr_knowledge SET content = ?, keywords = ?, updated_at = NOW() WHERE id = ?")
...
if ($has_new) {
    ...
    $db->prepare("INSERT INTO obr_knowledge (type, title, content, meta_json, keywords, period_start) VALUES ('experience', ?, ?, ?, ?, ?)")
...
if ($transversal) {
    ...
    $keywords = mb_strtolower($competency);
    $db->prepare("INSERT INTO obr_knowledge (type, title, content, meta_json, keywords) VALUES ('competence', ?, ?, ?, ?)")
...
if (isset($enrichmentData[$comp_index])) {
    ...
    $db->prepare("UPDATE obr_applications SET enrichment_data = ? WHERE id = ?")
