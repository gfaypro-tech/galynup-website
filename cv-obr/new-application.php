...
function buildKnowledgeBlock(PDO $db): string {
    $entries = $db->query("SELECT type, title, content, meta_json FROM obr_knowledge WHERE is_active = 1 ORDER BY type, IFNULL(period_start, 0) DESC, created_at DESC")->fetchAll();
...
elseif ($step === 3):
    ...
    $experiences  = $db->query("SELECT id, title, meta_json FROM obr_knowledge WHERE type = 'experience' AND is_active = 1 ORDER BY IFNULL(period_start, 0) DESC, created_at DESC")->fetchAll();
?>
<div class="card">
  <div class="card-title">🎯 Étape 3 — Matching & Enrichissement</div>
